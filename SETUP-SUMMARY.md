...
Your monorepo is now configured to publish `@mng/stencil-components` and `@mng/react-components` to npm.

### Files Modified

#### 1. `packages/stencil-components/package.json`
- ✓ Updated package name: `stencil-components` → `@mng/stencil-components`
- ✓ Added `publishConfig` with `"access": "public"`

#### 2. `packages/stencil-components/stencil.config.ts`
- ✓ Added `componentCorePackage: '@mng/stencil-components'` to React output target
- ✓ This ensures React wrappers use the correct scoped imports

#### 3. `packages/react-components/package.json`
- ✓ Updated package name: `react-components` → `@mng/react-components`
- ✓ Updated description
- ✓ Changed dependency: `stencil-components: "*"` → `@mng/stencil-components: "^0.0.1"`
...
- ✓ Updated dependency: `react-components: "*"` → `@mng/react-components: "*"`

#### 5. `packages/my-app/src/App.tsx`
- ✓ Updated import: `from 'react-components'` → `from '@mng/react-components'`
...
- ✓ Now imports from `@mng/stencil-components` instead of `stencil-components`
...
2. Have access to publish under the `@mng` scope
...
- Stencil Components: `https://www.npmjs.com/package/@mng/stencil-components`
- React Components: `https://www.npmjs.com/package/@mng/react-components`
...
- **Publish Order**: Always publish `@mng/stencil-components` before `@mng/react-components`
...
- **Scope Access**: The `@mng` scope must allow public packages (configured in `publishConfig`)

## Rollback Plan

If something goes wrong after publishing:
- You can unpublish within 72 hours: `npm unpublish @mng/package-name@version`
- After 72 hours, you can only deprecate: `npm deprecate @mng/package-name@version "reason"`
